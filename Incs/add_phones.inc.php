<?php
	...
	$error = '';
if (isset($_POST["p_Name"]) && ($error != "<span style='color:green;'>Success! Upload another item?</span>")){
		...
		if	(($extension=="jpg"||$extension=="jpeg"||$extension=="png")&&($type=="image/jpeg"||$type=="image/jpg"||$type=="image/png")){
			...
			if ($size<=$max_size){
				...
				if (move_uploaded_file($tmp_name, $location.$p_name_pic.".".$extension)){
					...
					`Brand`,`Display`,`Internal Storage`, `Front Camera`, `Rear Camera`) VALUES (
					...
					if(!(mysql_query($query))){
						...
						$item_id = queryULT('id','phones','Name',mysql_real_escape_string($p_name));
...
?>
<div class="results_heading_2"style="width:968px; top:10px; left:10px; position:relative;">Add A Phone</div>
