...
&& (isset($_POST['pageUrl']) && !empty($_POST['pageUrl']))){
	...
	if($mysql_num_rows>=1){
		...
		$query_2 = "UPDATE `likes_reviews` SET `like`='1', `dislike`='0' WHERE 
			`category`='".$category."' AND 
			`user`='".$_SESSION['user_username']."' AND
			`pageUrl`='".$pageUrl."' AND
			`review_id`='".$review_id."'";
		mysql_query($query_2);
		if($if_dislike==1){
		$query = "UPDATE `reviews` SET `likes`=`likes`+1, `dislikes`=`dislikes`-1 WHERE `id`='".$review_id."' AND `category`='".$category."' AND `pageURL`='".$pageUrl."'";
		} else {
		$query = "UPDATE `reviews` SET `likes`=`likes`+1 WHERE `id`='".$review_id."' AND `category`='".$category."' AND `pageURL`='".$pageUrl."'";
		}
		mysql_query($query);
	} else {
