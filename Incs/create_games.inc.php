<div class="results_heading_2"style="width:968px; top:10px; left:10px; position:relative;">Create Game Comparison</div>
<?php // LATEST
...
for ($x=0;$x<count($choice_id);$x++){
	...
	<?php if (($x%3)==0){echo "<tr>";}?>
		<td class="tdCreate showDetails" style=" background-color:white;">
			...
			?>">
			<?php } ?>
			<div class="compBox" style="height:164px; text-decoration:none;">
				<table  <?php if($choice_id[$x]==$choice1){ echo "id='this_one'"; }?> class="comparison_select tdTableCreate">
					<tr style="text-align:center; z-index:1;">
						<td style="max-width:217px; height:25px;"><?php echo "<span style='color:#4D4D4D; font-size:15px;'>".$choice_name[$x]."</span>"; ?></td>
					</tr>
					<tr style="z-index:1;">
						<td id="pic" style="z-index:1; width:217px;">
							<img alt="<?php echo $choice_name[$x]; ?>" src="Pictures/<?php echo ucfirst($comparison_category)."/".$choice_pic_name[$x]; ?>" style="max-width:217px; max-height:217px;background-color:white; z-index:1" />
						</td>
					</tr>
			...
			<div  id="search_options_games" style="border:1px solid #CCCCCC; position:relative;top:-4px; border-top:0;">
				<table>
					...
					<form method="GET" action="<?php echo curPageUrl();?>">
						<input type="hidden" readonly="true" name="cat" value="games-c" />
						<?php if (strpos($pageURL, "choice1=") !== false){?>
							<input type="hidden" readonly="true" name="choice1" value="<?php echo $choice1."-c1";?>"/>
						<?php } ?>
						<tr>
							<td style="position:relative; left:5px; text-align:center;"><div class="divSearchFields" style="position:relative; left:45px;">Search For A Game</div></td>
						</tr>
						<tr>
					...
					<form method="GET" action="<?php echo curPageUrl();?>">
						...
						<?php if (strpos($pageURL, "choice1=") !== false){?>
							<input type="hidden" readonly="true" name="choice1" value="<?php echo $choice1."-c1";?>"/>
						<?php } ?>
						<?php require("Incs/games_searchTable.inc.php");?>
					</form>
				</table>
			</div>
		</td>
