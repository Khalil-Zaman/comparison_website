	...
	if(loggedin()){
		...
		if($mysql_num_rows>=1){
			...
			if($query_row['like']!=1){
				...
				$query_2 = "UPDATE `likes_spec_reviews` SET `like`='1', `dislike`='0' WHERE 
					`item_id`='".$item_id."' AND 
					`user`='".$_SESSION['user_username']."' AND
					`review_id`='".$review_id."' AND
					`category`='".$category."'";
				mysql_query($query_2);
				if($if_dislike==1){
				$query = "UPDATE `spec_reviews` SET `likes`=`likes`+1, `dislikes`=`dislikes`-1 WHERE `item_id`='".$item_id."' AND `category`='".$category."' AND `id`='".$review_id."'";
				} else {
				$query = "UPDATE `spec_reviews` SET `likes`=`likes`+1 WHERE `item_id`='".$item_id."' AND `category`='".$category."' AND `id`='".$review_id."'";
				}
				mysql_query($query);
			}
		} else {
