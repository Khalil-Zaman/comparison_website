	...
	if(($query_row["verified"]==0)&&($query_row["delete"]==0)){
		...
		if(!$query_run_2){
			...
			if (file_exists($filename)) {
				//unlink($_SERVER['DOCUMENT_ROOT']."/Pictures/".ucfirst($category)."/".$pic_name);//ONLINE
			...
			if($category=="games"){
				...
				if (file_exists($filename)) {
					//unlink($_SERVER['DOCUMENT_ROOT']."/Website/Pictures/".$category."/Icons/".queryULT("icon_img","games","id",$id));//PRACTISE
			...
			'ADMIN',
			'".time()."'
			'We\'re sorry, you\'re ".$category.", ".$NameItem." was not verified.',
			'0')";
