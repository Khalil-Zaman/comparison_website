...
if(isset($_POST["category"]) && isset($_POST["url"])){
	...
	if (loggedin()){
		...
		if($mysql_num_rows>=1){
			...
			$query_2 = "UPDATE `likes` SET `left`='0', `right`='1' WHERE 
				`category`='".$category."' AND 
				`username`='".$_SESSION['user_username']."' AND
				`pageURL`='".$comparison_url."'";
			mysql_query($query_2);
		...
		if(isset($_SESSION[$category."_".$comparison_url.'_left'])){ // CHECK TO SEE IF USER HAS ALREADY LIKED LEFT SIDE
			if($_SESSION[$category."_".$comparison_url.'_left']=='1'){
				$query = "UPDATE `compare` SET 
				...
				WHERE `pageURL`='".$comparison_url."' AND `category`='".$category."'";
				mysql_query($query);
				$_SESSION[$category."_".$comparison_url.'_left']='0';
				$_SESSION[$category."_".$comparison_url.'_right']='1';
