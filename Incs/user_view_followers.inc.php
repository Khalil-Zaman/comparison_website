<?php
$query = "SELECT * FROM `following` WHERE `user_followed`='".$username."' AND `unfollow`='0' ORDER BY `time` DESC";
...
if ($query_num_rows>0){
	...
	while ($query_row = mysql_fetch_assoc($query_run)){
		$followersName[$x] = $query_row['user_following'];
		$x++;
	}
}
if($query_num_rows>0){
...
<div style="position:relative;">
	<table style="border-collapse:collapse: border-spacing:0; position:relative; border:1px solid #CCCCCC; width:739px; border-bottom:0;">
		<tr>
			<td>
				<table style="border-spacing:1px; <?php if($query_num_rows>=6){ echo 'margin:auto;'; }?>">
			<?php for ($x=0; 
