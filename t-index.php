...
while ($query_row = mysql_fetch_assoc($query_run)){
	...
	$TAI1[$x] = $comparison_category_ar[$x].$extension[$x].queryULT($pic[$x],$comparison_category_ar[$x],"id",$choice1[$x]);
	$TAI2[$x] = $comparison_category_ar[$x].$extension[$x].queryULT($pic[$x],$comparison_category_ar[$x],"id",$choice2[$x]);
	$x++;	
}
...
while ($query_row = mysql_fetch_assoc($query_run)){	
	if (($x+1) <= 20){
		...
		$fpPIC1G[$x] = $comparison_category.$extensionG.queryULT("icon_img",$comparison_category,"id",$fpC1G[$x]);
		$fpPIC2G[$x] = $comparison_category.$extensionG.queryULT("icon_img",$comparison_category,"id",$fpC2G[$x]);
		$fpFullPIC1G[$x] = $comparison_category."/".queryULT("pic_name",$comparison_category,"id",$fpC1G[$x]);
		$fpFullPIC2G[$x] = $comparison_category."/".queryULT("pic_name",$comparison_category,"id",$fpC2G[$x]);
		$x++;
	}
}
?>
...
while ($query_row = mysql_fetch_assoc($query_run)){	
	if (($x+1) <= 20){
		...
		$fpPIC1L[$x] = $comparison_category.$extensionL.queryULT("pic_name",$comparison_category,"id",$fpC1L[$x]);
		$fpPIC2L[$x] = $comparison_category.$extensionL.queryULT("pic_name",$comparison_category,"id",$fpC2L[$x]);
		$x++;
	}
}
?>
...
while ($query_row = mysql_fetch_assoc($query_run)){	
	if (($x+1) <= 20){
		...
		$fpPIC1P[$x] = $comparison_category.$extensionP.queryULT("pic_name",$comparison_category,"id",$fpC1P[$x]);
		$fpPIC2P[$x] = $comparison_category.$extensionP.queryULT("pic_name",$comparison_category,"id",$fpC2P[$x]);
		$x++;
	}
}
?>
				...
				<table class="compTableTable">
					<tr>
						<td colspan="2" class="compTableTableTd">
							<img class="compTableTopImg" src="Pictures/<?php echo $comparison_category_ar[$x]."/".queryULT("pic_name",$comparison_category_ar[$x],"id",$choice1[$x]);?>">
						</td>
						<td colspan="2" >
							<div class="bigCompLine"></div>
						</td>
						<td colspan="2" class="compTableTableTd">
							<img class="compTableTopImg" src="Pictures/<?php echo $comparison_category_ar[$x]."/".queryULT("pic_name",$comparison_category_ar[$x],"id",$choice2[$x]);?>">	
						</td>
					</tr>
					<tr>
										...
										<div class="fpItemImgContainer" >
											<div class="fpItemImg" >
												<a href="<?php echo "details_info.php?=".$comparison_category_TopItem[$x]."-".$id_TopItem[$x]."-c";?>">
													<img src="Pictures/<?php echo $comparison_category_TopItem[$x].$extension_TopItem[$x].$pic_TopItem[$x];?>" class="itemPic"/>
												</a>
											</div>
										</div>
									</li>
	...
	if (($x%3)==0){echo "<tr>";};?>
		...
		<a class="displayLatestResultsHref" href="<?php echo "compare.php?".$comparison_category_latest[$x]."-c".$pageURL_latest[$x]."-gc";?>">
			<table class="displayLatestResultsTable2">
				<tr>
				<td colspan="2" >
					<img src="Pictures/<?php echo $comparison_category_latest[$x].$extension_latest[$x].queryULT($pic_latest[$x],$comparison_category_latest[$x],"id",$choice1_latest[$x]);?>" class="compResultImgs compResultImgLeft">
				</td>
				<td colspan="2" >
					<div class="smallCompLine"></div>
				</td>
				<td  colspan="2">
					<img src="Pictures/<?php echo $comparison_category_latest[$x].$extension_latest[$x].queryULT($pic_latest[$x],$comparison_category_latest[$x],"id",$choice2_latest[$x]);?>" class="compResultImgs">
				</td>
				</tr>
	...
	if (($x%3)==0){echo "<tr>";};?>
		...
		<a class="displayLatestResultsHref" href="compare.php?<?php echo $category[$x]."-c".$pageURL_search[$x]."-gc";?>">
			<table class="displayLatestResultsTable2">
				<tr>
				<td colspan="2">
					<img src="Pictures/<?php echo $category[$x].$ext[$x].queryULT($select[$x],$category[$x],"id",$choice1_search[$x]);?>" class="compResultImgs compResultImgLeft">
				</td>
				<td colspan="2" >
					<div class="smallCompLine"></div>
				</td>
				<td  colspan="2">
					<img src="Pictures/<?php echo $category[$x].$ext[$x].queryULT($select[$x],$category[$x],"id",$choice2_search[$x]);?>" class="compResultImgs">
				</td>
				</tr>
				<tr class="name_details nameDetailsAdj">
					<td colspan="6" style="border-collapse:collapse;">
		...
		<td style="background-color:white; ">
			<div  id="search_options_games" class="searchBar1Options">
				...
				<form method="GET" action="<?php echo curPageUrl();?>">
					...
					<tr>
						<td class="sB1TableTdTitles2" style="position:relative; top:10px;">
							Choice 1
						</td>
					</tr>
					<tr>
						<td class="sB1TableTdO">Genre</td>
					</tr>
					<tr>
						<td class="sB1TableTd2O">
						<select name="games_Genre_Choice_1" class="sBselect">
					...
					<tr>
						<td class="sB1TableTdO">Platform</td>
					</tr>
					<tr>
						<td class="sB1TableTd2O">
						<select name="games_Platform_Choice_1" class="sBselect">
					...
					<tr>
						<td class="sB1TableTdTitlesVs">
							VS
						</td>
					</tr>
					
					<tr>
						<td class="sB1TableTdTitles2">
							Choice 2
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td class="sB1TableTd2O">
						<select name="games_Genre_Choice_2" class="sBselect">
							<?php for ($x=1;$x<=count($Genre);$x++){?>
							<option value="<?php echo $Genre[$x-1]?>"><?php echo $Genre[$x-1] ?></option>
							<?php } ?>
						</select>
					...
					<tr>
						<td class="sB1TableTd2O">
						<select name="games_Platform_Choice_2" class="sBselect">
							<?php for ($x=1;$x<=count($Platform);$x++){?>
							<option value="<?php echo $Platform[$x-1]?>"><?php echo $Platform[$x-1] ?></option>
							<?php } ?>
						</select>										<!-- FIX PAGE NUMBERS -->
						</td>
					</tr>
					<input type="hidden" readonly="true" value="<?php echo $page_number;?>" name="page"/>
					<tr>
						<td class="sB1TableSubmit">
			...
			<div id="search_options_laptops" class="searchBar2Options">
				<table>
					<form method="GET" action="<?php echo curPageUrl();?>">
						...
						<tr>
							<td class="sB1TableTdO">Brand</td>
						</tr>
						<tr>
							<td class="sB1TableTd2O">
							<select name="laptops_Brand_Choice_1" class="sBselect">
						...
						<tr >
							<td class="sB1TableTdO">Ram</td>
						</tr>
						<tr>
							<td class="sB1TableTd2O">
							<select name="laptops_Ram_Choice_1" class="sBselect">
						...
						<tr>
							<td class="sB1TableTdO">Processor</td>
						</tr>
						<tr>
							<td class="sB1TableTd2O">
							<select name="laptops_Processor_Choice_1" class="sBselect">
						...
						<tr>
							<td class="sB1TableTd2O">
							<select name="laptops_Brand_Choice_2" class="sBselect">
								<?php for ($x=1;$x<=count($Brand);$x++){?>
								<option value="<?php echo $Brand[$x-1]?>"><?php echo $Brand[$x-1] ?></option>
								<?php } ?>
							</select>
						...
						<tr>
							<td class="sB1TableTd2O">
							<select name="laptops_Ram_Choice_2" class="sBselect">
								<?php for ($x=1;$x<=count($Ram);$x++){?>
								<option value="<?php echo $Ram[$x-1]?>"><?php echo $Ram[$x-1] ?></option>
								<?php } ?>
							</select>
						...
						<tr>
							<td class="sB1TableTd2O">
							<select name="laptops_Processor_Choice_2" class="sBselect">
								<?php for ($x=1;$x<=count($Processor);$x++){?>
								<option value="<?php echo $Processor[$x-1]?>"><?php echo $Processor[$x-1] ?></option>
								<?php } ?>
							</select>
			...
			<div id="search_options_phones" class="searchBar3Options">
				<table>
					<form method="GET" action="<?php echo curPageUrl();?>">						
						...
						<tr>
							<td class="sB1TableTd2O">
							<select name="phones_Brand_Choice_1" class="sBselect">
						...
						<tr>
							<td class="sB1TableTdO">Display</td>
						</tr>
						<tr>
							<td class="sB1TableTd2O">
							<select name="phones_Display_Choice_1" class="sBselect">
						...
						<tr>
							<td class="sB1TableTdO">Storage</td>
						</tr>
						<tr>
							<td class="sB1TableTd2O">
							<select name="phones_Storage_Choice_1" class="sBselect">
								<?php for ($x=1;$x<=count($Phones_Storage);$x++){?>
								<option value="<?php echo $Phones_Storage[$x-1]?>"><?php echo $Phones_Storage[$x-1] ?></option>
								<?php } ?>
							</select>
							</td>
						</tr>
						<tr>
							<td class="sB1TableTdO">Front Camera</td>
						</tr>
						<tr>
							<td class="sB1TableTd2O">
							<select name="phones_Front_Camera_Choice_1" class="sBselect">
						...
						<tr>
							<td class="sB1TableTdO">Rear Camera</td>
						</tr>
						<tr>
							<td class="sB1TableTd2O">
							<select name="phones_Rear_Camera_Choice_1" class="sBselect">
						...
						<tr>
							<td class="sB1TableTd2O">
							<select name="phones_Brand_Choice_2" class="sBselect">
								<?php for ($x=1;$x<=count($Phones_Brand);$x++){?>
								<option value="<?php echo $Phones_Brand[$x-1]?>"><?php echo $Phones_Brand[$x-1] ?></option>
								<?php } ?>
							</select>
						...
						<tr>
							<td class="sB1TableTd2O">
							<select name="phones_Display_Choice_2" class="sBselect">
								<?php for ($x=1;$x<=count($Phones_Display);$x++){?>
								<option value="<?php echo $Phones_Display[$x-1]?>"><?php echo $Phones_Display[$x-1] ?></option>
								<?php } ?>
							</select>
						...
						<tr>
							<td class="sB1TableTd2O">
							<select name="phones_Storage_Choice_2" class="sBselect">
								<?php for ($x=1;$x<=count($Phones_Storage);$x++){?>
								<option value="<?php echo $Phones_Storage[$x-1]?>"><?php echo $Phones_Storage[$x-1] ?></option>
								<?php } ?>
							</select>
						...
						<tr>
							<td class="sB1TableTd2O">
							<select name="phones_Front_Camera_Choice_2" class="sBselect">
								<?php for ($x=1;$x<=count($Phones_Front_Camera);$x++){?>
								<option value="<?php echo $Phones_Front_Camera[$x-1];?>"><?php echo $Phones_Front_Camera[$x-1];?></option>
								<?php } ?>
							</select>
						...
						<tr>
							<td class="sB1TableTd2O">
							<select name="phones_Rear_Camera_Choice_2" class="sBselect">
								<?php for ($x=1;$x<=count($Phones_Rear_Camera);$x++){?>
								<option value="<?php echo $Phones_Rear_Camera[$x-1];?>"><?php echo $Phones_Rear_Camera[$x-1];?></option>
								<?php } ?>
							</select>
