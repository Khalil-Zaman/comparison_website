...
$choice1_name = array();
$choice2_name = array();
$query = "SELECT * FROM  `compare` WHERE `category`='games' ORDER BY `pageViews_day` DESC";
...
while ($query_row = mysql_fetch_assoc($query_run)){	
	if (($x+1) <= 20){
		...
		if ($pageViews_day != 0){
			...
			$query = "SELECT * FROM  `compare` WHERE `category`='games' ORDER BY `id` DESC ";
			$query_run = mysql_query($query);
			$query_num_rows = mysql_num_rows($query_run);
			while ($x2_w<=$x2){
				$query_row = mysql_fetch_assoc($query_run);
...
$query = "SELECT * FROM `compare` WHERE `category`='games'";
$query_run = mysql_query($query);
$query_num_rows = mysql_num_rows($query_run);
...
$query = "SELECT * FROM  `compare` WHERE `category`='games' ORDER BY `time_added` DESC LIMIT ".(($page_number-1)*$num_comparisons_pp).", ".$num_comparisons_pp;
$query_run = mysql_query($query);
while($query_row = mysql_fetch_assoc($query_run)){
...
<div class="results_heading_2 topTitle">Top Game Comparisons Today</div><!-- TOP COMPARISONS DISPLAY BOX -->
<table class="compTable">
	<tr>
		<td colspan="3" rowspan="1" class="border" id="showing_comparison"><!-- GAME TOP -->
		<div class="compTopPaddingBoxGames">
		<!-- TOP 3 -->
		<?php
		$top_game_id = 1;
		for ($x = 0; ($x+1)<=9;$x++){
		?>
			<a href="<?php echo "compare.php?".$comparison_category."-c".$pageURLArray[$x]."-gc";?>" id="<?php echo"game_top_".$top_game_id;?>" class="front_top_comp" >
				<table class="compTableTable" style="position:relative; left:5px;">
					<tr>
						<td colspan="2" class="compTableTableTdGames">
							<img class="compTableTopImg" src="Pictures/<?php echo ucfirst($comparison_category)."/".queryULT("pic_name",$comparison_category,"id",$choice1[$x]);?>" width="325" height="183"/>
						</td>
						<td colspan="2" >
							<div style="height:182px; background-color:green; width:1px; margin-top:-2px;"></div>
						</td>
						<td colspan="2" class="compTableTableTdGames">
							<img class="compTableTopImg" src="Pictures/<?php echo ucfirst($comparison_category)."/".queryULT("pic_name",$comparison_category,"id",$choice2[$x]);?>" width="325" height="183"/>
						</td>
					</tr>
					<tr>
			...
			?>
				<tr>
					...
					<td class="top_results_comparison">
						<a  href="compare.php?games-c<?php echo $pageURLArray[$x]."-gc";?>">
	...
	<tr><!-- GAME TOP DP -->
		...
		<a href="<?php echo "compare.php?".$comparison_category."-c".$pageURLArray[$x]."-gc";?>" id="<?php echo"game_top_dp_".$top_game_id;?>" class="comp_dp">
			<table class="comp_front_table_comps">
				<tr>
				<td colspan="2">
					<img class="compImg" src="Pictures/<?php echo ucfirst($comparison_category)."/Icons/".queryULT("icon_img",$comparison_category,"id",$choice1[$x]);?>">
				</td>
				<td colspan="2" >
					<div style="height:100px; background-color:green; width:1px;"></div>
				</td>
				<td  colspan="2">
					<img class="compImg" src="Pictures/<?php echo ucfirst($comparison_category)."/Icons/".queryULT("icon_img",$comparison_category,"id",$choice2[$x]);?>">
				</td>
				</tr>
			</table>
		</a>
...
<div style="position:relative; float:right; top:30px; right:10px; padding-bototm:100px;"><!-- RESULTS -->
<?php if ($Search==true){ ?><!-- DISPLAYING SEARCH RESULTS -->
<div class="results_heading_2">Search Results</div>
<table style="border-collapse:collapse; border-spacing: 0; position:relative;top:10px;">
<?php
for ($x=0;$x<count($choice1_name_search);$x++){
	if (($x%3)==0){echo "<tr>";};?>
		<td class="comp_select">
		<div class="compBox">
		<a class="displayLatestResultsHref" href="compare.php?games-c<?php echo $pageURL_search[$x]."-gc";?>">
			<table class="displayLatestResultsTable2">
				<tr>
				<td colspan="2" >
					<img src="Pictures/<?php echo ucfirst($comparison_category)."/Icons/".queryULT("icon_img",$comparison_category,"id",$choice1_search[$x]);?>" class="compResultImgs compResultImgLeft">
				</td>
				<td colspan="2" >
					<div class="smallCompLine"></div>
				</td>
				<td  colspan="2">
					<img src="Pictures/<?php echo ucfirst($comparison_category)."/Icons/".queryULT("icon_img",$comparison_category,"id",$choice2_search[$x]);?>"  class="compResultImgs">
				</td>
				</tr>
...
<?php if ($Search==false){?><!-- DISPLAY LATEST RESULTS -->
<div class="results_heading_2">Newest Game Comparisons</div>
<table style="border-collapse:collapse; border-spacing: 0; position:relative; top:10px;">
<?php
for ($x=0;$x<count($pageUrl_latest);$x++){
	if (($x%3)==0){echo "<tr>";};?>
		<td class="comp_select">
		<div class="compBox">
		<a class="displayLatestResultsHref" href="compare.php?games-c<?php echo $pageUrl_latest[$x]."-gc";?>">
			<table class="displayLatestResultsTable2">
				<tr>
				<td colspan="2" >
					<img src="Pictures/<?php echo ucfirst($comparison_category)."/Icons/".queryULT("icon_img",$comparison_category,"id",$choice1_latest[$x]);?>" class="compResultImgs" style="margin-left:8px;">
				</td>
				<td colspan="2" >
					<div class="smallCompLine"></div>
				</td>
				<td  colspan="2">
					<img src="Pictures/<?php echo ucfirst($comparison_category)."/Icons/".queryULT("icon_img",$comparison_category,"id",$choice2_latest[$x]);?>" class="compResultImgs">
				</td>
				</tr>
				<tr class="name_details nameDetailsAdj">
					<td colspan="6" style="border-collapse:collapse;">
...
<div style="position:relative; float:left; top:30px; left:10px;">
<table style="background-color:white; width:230px; border-collapse:collapse;"><!-- SEARCH BAR -->
	...
	<div style="position:relative; background-color:#E9E9E9; top:6px;padding:14px; text-align:center; border:1px solid #CCCCCC;">
		<div>
